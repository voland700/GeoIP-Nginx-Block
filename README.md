...
Для блокировки доступа используются базы данных GeoLite. Ресурс GeoLite в настоящее время является платным и требует регистрации, кроме того, пользователям некоторых стран, в том числе РФ, доступ запрешен.

К счастью, есть регулярно обновляемые клоны, со свободных доступом, без регистрации и оплаты. К примеру Git репозиторий с актуальными mmdb от P3TERX:

https://github.com/P3TERX/GeoLite.mmdb?tab=readme-ov-file

В данном GIT репозитории содержаться ссылки на базы данных geo ip по ASN - провайдеров, городам и странам. Для блокировки доступа к сайту по странам, используем ссылку из указанного репозитория на базу:  GeoLite2-Country.mmdb.
...
## Добавляем скрипт autoupdate_db.sh в cron для запуска раз в неделю
